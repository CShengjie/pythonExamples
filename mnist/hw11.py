...
def generateSets(testLabel):
    l1 = []
    l2 = []
    while len(l1) < 10:
        x = random.randint(0,len(testLabel)-1)
        y = random.randint(0,len(testLabel)-1)
        if(x != y and testLabel[x]==0):
            l1.append([x,y])
    while len(l2) < 10:
        x = random.randint(0,len(testLabel)-1)
        y = random.randint(0,len(testLabel)-1)
        if(x != y and testLabel[x]!=testLabel[y]):
            l2.append([x,y])
    return l1,l2
...
test1,test2 = generateSets(testLabel)
test1Set = [[testImg[i[0]],[testImg[i[1]]]] for i in test1]
test2Set = [[testImg[i[0]],[testImg[i[1]]]] for i in test2]

print(testImg[1])
# for i in range(10):
#     for i in range(8):
#         test = vae.reconstruct(x_sample)

# output = np.empty((28*10,28*10))
